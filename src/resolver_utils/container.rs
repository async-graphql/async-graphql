...
use crate::{
    Context, ContextBase, ContextSelectionSet, Error, Name, OutputType, OutputTypeMarker, ServerError, ServerResult, Value, extensions::ResolveInfo, parser::types::Selection
};
...
#[cfg_attr(feature = "boxed-trait", async_trait::async_trait)]
impl<T: ContainerType + OutputTypeMarker, E: Into<Error> + Send + Sync + Clone> ContainerType for Result<T, E> {
    async fn resolve_field(&self, ctx: &Context<'_>) -> ServerResult<Option<Value>> {
        match self {
